...
- pnpm workspace モノレポ: `packages/core`(ドメイン/DB/services/providers/auth) + `apps/web`(UI+API) + `apps/mcp`(**MCP サーバ, 26ツール・本番稼働・claude.ai 接続済**) + `tools`(OAuth CLI / probe)
...
- **M2(完了)**: `apps/mcp` MCP サーバ(26ツール: 記録→分析→週次サマリ→コンディション信号→ルーティン計画→エネルギー収支→GH反映→取消)、`MCP_SHARED_SECRET`+IP 二次防御、claude.ai Custom Connector 接続、種目エイリアス辞書。
