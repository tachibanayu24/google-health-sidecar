...
- pnpm workspace モノレポ: `packages/core`(ドメイン/DB/services/providers/auth) + `apps/web`(UI+API) + `apps/mcp`(**MCP サーバ, 31ツール・本番稼働・claude.ai 接続済**) + `tools`(OAuth CLI / probe)
...
- **M2(完了)**: `apps/mcp` MCP サーバ(31ツール: 記録→分析→週次サマリ→コンディション信号→ルーティン計画→適応型TDEE/停滞/食事×回復相関→エネルギー収支→食事スコア(マクロ目標適合度レーダー)→ワークアウトメモ(set_workout_note・AIコメント)→GH反映→取消)、`MCP_SHARED_SECRET`+IP 二次防御、claude.ai Custom Connector 接続、種目エイリアス辞書。
