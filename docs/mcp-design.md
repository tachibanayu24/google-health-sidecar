...
> - **ツールは21本**で稼働。本書の R/W/D カタログ(`get_settings`・`get_exercise_history`・`get_muscle_volume`・`get_muscle_calendar`・`get_recent_sessions`・`get_recent_prs`・`search_exercises`・`autocomplete_foods`・`get_day`・`log_meal`・`log_workout`・`log_weight`・`set_nutrition_target`・`delete_recent_log`)に加え、**`get_training_frequency`(§5.5-G 実装)・`get_meal_presets`・`save_meal_preset`・`log_preset`・`delete_meal_preset`・`log_meal_photo`・`get_weekly_summary`(直近7日の総合サマリ)** を実装。`log_workout` の返り値 `newPrs` は新自己ベスト(name/value/prevValue/isProvisional)を含み、Claude が会話で称えられる。
