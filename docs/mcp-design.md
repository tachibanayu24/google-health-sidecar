...
> - **ツールは35本**で稼働(週次レポート4本=`get_week_review_data`/`save_weekly_report`/`get_weekly_report`/`get_weekly_reports` を追加。AI が前週をヒアリングのうえ生成・保存。`docs/weekly-report-design.md`)。ルーティン4本(`get_routines`/`get_routine`/`save_routine`/`delete_routine`, design.md §8.10)+ 派生指標read 4本(`get_nutrition_status`=適応型TDEE逆算 / `get_plateau_indicators`=e1RM停滞検知 / `get_meal_recovery_correlation`=食事×翌朝回復の中央値差, design.md §8.11 / `get_nutrition_score`=食事のマクロ目標適合度スコア・レーダー, docs/nutrition-scoring-design.md。**質(脂質の質/GL/食事の質)は採点せず食品名を返す→AIが会話で判断**)+ **`set_workout_note`(ワークアウトにメモ/AIコメント。author='ai' で UI/export に「AI」ラベル・最大200文字・単一メモ欄 last-writer-wins)** を追加。本書の R/W/D カタログ(`get_settings`・`get_exercise_history`・`get_muscle_volume`・`get_muscle_calendar`・`get_recent_sessions`・`get_recent_prs`・`search_exercises`・`autocomplete_foods`・`get_day`・`log_meal`・`log_workout`・`log_weight`・`set_nutrition_target`・`delete_recent_log`)に加え、**`get_training_frequency`(§5.5-G 実装)・`get_meal_presets`・`save_meal_preset`・`log_preset`・`delete_meal_preset`・`log_meal_photo`・`get_weekly_summary`(直近7日の総合サマリ)・`get_readiness`(コンディション信号=個人ベースライン比の相対逸脱 + 部位別 急性/慢性ボリューム比 muscleLoad 同梱, design.md §8.8/§8.9)** を実装。`log_workout` の返り値 `newPrs` は新自己ベスト(name/value/prevValue/isProvisional)を含み、Claude が会話で称えられる。
...
read 系:
  ...
  get_nutrition_status { windowDays?(既定28) }              // §8.11 適応型TDEE逆算(遵守ゲート)
  get_plateau_indicators { windowDays?(既定56) }            // §8.11 種目別e1RM停滞検知
  get_meal_recovery_correlation { days?(既定28) }           // §8.11 食事×翌朝回復(n と中央値差のみ)
  get_nutrition_score  { date?(YYYY-MM-DD, 既定 今日JST) }  // 食事スコア(5軸×目標適合度・1日/カテゴリ・食品名同梱・質は未採点)
