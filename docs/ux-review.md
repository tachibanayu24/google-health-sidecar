...
## 今後の UX 改善(採用分・優先順)
1. **情報設計(IA)の再構成**: 「1画面に詰め込みすぎ」解消。ベンチマークアプリ参照でボトムナビ+画面責務から作り直し中(2026-06-01)。
2. **品質・運用(CI除く)**: D1バックアップ・KV監視・ALLOWED_SUB・PKCE。
   - ※ オフライン送信キュー・歩数日次集計・service層テスト・Zod は導入済。
