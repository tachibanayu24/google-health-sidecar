...
## 今後の UX 改善(優先順)
1. **種目マスタ第二弾**: free-exercise-db(800+)取込 + 図鑑(画像/手順)。日本語名は LLM バッチ補完。
2. **ワークアウトのさらなる強化**: スーパーセット、セット並べ替え、過去セッションの in-place 編集(現状は削除+再記録)。
3. **歩数の日次集計**: GH の分単位 interval を日次合計に集計して Home/センシングに復帰(現状除外)。
4. **PWA リッチ化**: オフライン下書き(IndexedDB アウトボックス)、インストール導線バナー、スプラッシュ最適化。
5. **周径・進捗写真**(§1.4): body_measurements 入力、R2 写真(M2+)。
