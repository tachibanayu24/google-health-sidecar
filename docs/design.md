...
- **将来(M4以降)**: e1RM回帰直線での停滞検知→デロード提案 / 体重(GH)× PFC(D1)× 週間トレボリューム(D1)× 推定消費kcal の統合相関ビュー(周径も軸に) / HRV・安静時心拍・睡眠 × 直近トレ刺激でオーバーリーチ警告(**回復信号部分は §8.8 Readiness で実装済**。トレ刺激=ACWR との合成は将来)。
...
### 8.8 Readiness(コンディション信号)【実装済: 2026-06-03 / enhancements.md ④⑤】
回復センシング(GH)を個人ベースライン比で信号化し、UI(からだ画面)と MCP `get_readiness` の両方に同一の決定的純関数を供給する。横断調査(11エージェント workflow・各主張を反証検証, 全4次元 holdsUp=true)に基づき、**学術的に確立された「測定モニタリング手法」として実装。予測・処方・診断ツールではない**。出すのは「あなた自身の過去データに対する相対的な逸脱の事実」までで、ここを越えると即破綻する(ガードレールが成立の前提)。

- **配置**: 純関数 `packages/core/src/domain/readiness.ts`(DB/IO/LLM 非依存・単体テスト 12 件)。データ取得は `db/repositories/readiness.ts`(read 専用・§8.5 write 経路に無関係)。`/api/readiness` と MCP `get_readiness` が共有。
- **指標の階層**(検証強度順):
  - **中核 = 夜間 HRV(rMSSD)**: 単日はノイズ過大(CV≈30%)なため `ln(rMSSD)` → **直近7日ローリング平均**を当日代表値とする(Plews & Buchheit 2013/2014)。これを省くと「適当な計算」になる。窓内有効 4 日未満は不算出。
  - 補助 = 安静時心拍 / 呼吸数。文脈 = 皮膚温(夜間)/ 睡眠時間・効率(自分比の低下のみ)。
  - **判定除外**: SpO2(健常域の変動が計測誤差に埋もれる)・睡眠ステージ deep/REM(消費者デバイスの分類精度 κ0.2–0.46=PSG非等価)・VO2max。UI では参考表示に留める。
- **個人ベースライン**: 当日を除く直近最大60日の代表値系列の **中央値 ± MAD**(robust z = (x−median)/(1.4826·MAD) は SD 単位, Iglewicz & Hoaglin 1993)。平均±SD は小標本/外れ値で膨張するため不使用。MAD は指標別の下限でフロア(MAD=0 退化=過敏化を防ぐ)。表示値は ln 指標を exp で逆変換。
- **閾値**: SD 単位(HRV 黄1/赤2 など)。**呼吸数のみ査読由来の絶対閾値**(中央値比 +3/min=黄・+5/min=赤, Natarajan & Heneghan 2021)。閾値は文献慣習に基づく工学的選択で、運用しながら誤検知率を見て調整する前提。
- **統合 = N-of-M(偽の 0–100 合成スコアは作らない)**: 各指標を個別に緑/黄/赤へ信号化し、**2指標以上が同時に悪方向へ逸脱で全体赤**(Apple Vitals と同型)。単一指標の逸脱は黄止まり(単独で赤に昇格しない)。**HRV(中核)単独の赤は全体赤へ昇格**。商用合成スコアは独立検証ほぼ皆無(De Gruyter 2025)ゆえ独自重みを発明しない。
