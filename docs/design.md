...
| `resp-rate` | `daily-respiratory-rate` | health_metrics_and_measurements | `daily_metrics(metric='resp_rate', unit='/min')` | 【要検証: 正確なdataType IDをM0 discovery docで確定】 |
| `skin-temp` | `daily-skin-temperature` | health_metrics_and_measurements | `daily_metrics(metric='skin_temp_c', unit='celsius', 絶対℃)` | 【要検証: ID/絶対値仕様をM0確定】 |
...
| # | 項目 | 確度 | 暫定方針 / 検証アクション |
|---|---|---|---|
| 1 | **nutrition write が第三者クライアントに実grantされるか** | 【一部前進・残要検証】**2026-05-31 オーナーのGCPコンソールで `googlehealth.nutrition.writeonly` スコープの実在を確認**(「栄養データのGoogle Healthへの追加/編集/削除」)。残るは `nutrition-log` create エンドポイントが実際に200で通るか | scope は提供済が確定 → bootstrap で grant 取得 → `oauth:check` で create の200/403を最終確認。`FEATURE_GH_NUTRITION_PUSH` で flag化、403時はD1正本維持+push停止。scope 存在により当初の「成立しない可能性」リスクは大幅低下 |
...
| nutrition | `foodDisplayName` / `energy.kcal` | **`nutritionLog.foodName`** / `caloriesKcal` / `mealType`(BREAKFAST/LUNCH/DINNER/SNACK) |
| daily 値 | `value` | resting=`beatsPerMinute` / hrv=`averageHeartRateVariabilityMilliseconds` / spo2=`percentage` / vo2max=`vo2MaxMlPerKgPerMinute` |
...
| sleep | `sleep.totalDurationMinutes` 等 | **`sleep.summary.minutesAsleep`** + `stagesSummary[]{type,minutes}`、efficiency は導出(minutesAsleep/minutesInSleepPeriod) |
| recordingMethod | `MANUAL` | enum に MANUAL 無し → **`ACTIVELY_RECORDED`**。own-write 判定は `gh_datapoint_id` 一致を主に(recordingMethod非依存) |
| create 応答 | bare DataPoint | **Operation**(`response.name` 優先、直下 name フォールバック) |

**残トークン検証 openItems**(契約テストで最終確定): ① `dataSource.application` の Application object 正確形(現状 `{name}` best-effort)② Operation 応答の done/poll 要否 ③ nutrition `nutrients[]` の正確な形(`{nutrient,quantity:{grams}}` は要確認、400で判明)④ ObservationTimeInterval の civil time 採否 ⑤ daily の部分 `Date`(0)ハンドリング。いずれも D1 正本に影響せず、初回 pull/probe で確定 → mapper微修正で吸収。`mappers.ts` 冒頭に対応表を明記済。

**意義**: 「要検証(discovery pin)」とマークしていた最大の実装リスクを**トークン前に大幅解消**。残るのは実レスポンスの最終照合(契約テスト)のみ。vitest で int64文字列パース/Date型時刻/weightGrams/sleep summary を回帰固定(37 tests)。
